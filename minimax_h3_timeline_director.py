...
class MiniMaxH3TimelinePlanner(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            outputs=[
                ...
                io.Int.Output(display_name="Segment Frames"),
            ],
        )
    ...
    def execute(
        ...
        segments = config.get("segments", [])
        # 过滤出启用的分段（enabled !== false），记录原始索引
        active_segments = [(i, s) for i, s in enumerate(segments) if s.get("enabled") is not False]
        active_count = len(active_segments)
        # 确定当前要输出的启用分段的原始索引
        target_original_idx = None
        if prompt_index is not None:
            ai = max(0, int(prompt_index))
            if ai < active_count:
                target_original_idx = active_segments[ai][0]
        elif config.get("mode") == "timeline" and active_count > 0:
            # 无prompt_index时，取当前UI选中的分段（如果启用），否则取第一个启用分段
            active_ui = int(config.get("activeIndex", 0))
            target_original_idx = active_ui if any(i == active_ui for i, _ in active_segments) else active_segments[0][0]
        # 生成选中分段的plan
        if target_original_idx is not None:
            internal_index = target_original_idx + 1  # 内部从1开始
            selected_plan = _create_timeline_plan(
                timeline_data, width, height, generation_seconds, internal_index,
            )
        else:
            selected_plan = complete_plan
        # 提取当前段提示词
        timeline = complete_plan["timeline"]
        global_prompt = str(timeline.get("globalPrompt") or "").strip()
        if segments and config.get("mode") == "timeline" and target_original_idx is not None:
            seg_prompt = str(segments[target_original_idx].get("prompt") or "").strip()
            segment_prompt = seg_prompt if seg_prompt else global_prompt
        else:
            segment_prompt = global_prompt
        segment_frames = int(selected_plan.get("length", 0))
        return io.NodeOutput(
            selected_plan,
            _create_prompt_media_bundle(selected_plan),
            complete_plan,
            segment_prompt,
            segment_frames,
        )
